...
function isDirectoryPattern(pattern) {
    ...
    if (knownFilePatterns.has(pattern))
        return false;
...
function createDirectoryMatcher(pattern) {
    const normalized = normalizePattern(pattern);
    ...
    if (normalized.includes("*") || normalized.includes("?")) {
        ...
        return (path) => regex.test(path) || regex.test(path + "/");
    }
...
export class IgnoreEngine {
    directoryMatchers;
    fileMatchers;
    constructor(patterns = BUILTIN_IGNORE_PATTERNS) {
