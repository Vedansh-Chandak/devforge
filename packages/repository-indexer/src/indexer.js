import { resolve as resolvePath, basename as pathBasename, extname, relative, sep, posix, } from "node:path";
import { lstat, readdir } from "node:fs/promises";
import { RepositoryScanError } from "./types.js";
import { defaultIgnoreEngine } from "./ignore.js";
function asErrno(err) {
...
function classifyRootError(root, err) {
    switch (err.code) {
        case "ENOENT":
            return new RepositoryScanError("NOT_FOUND", root, `Repository path does not exist: ${root}`);
        case "ENOTDIR":
            return new RepositoryScanError("NOT_A_DIRECTORY", root, `Repository path is not a directory: ${root}`);
        case "EACCES":
        case "EPERM":
            return new RepositoryScanError("PERMISSION_DENIED", root, `Repository path is not readable due to permissions: ${root}`);
        default:
            return new RepositoryScanError("INVALID_ROOT", root, `Unable to access repository path (${err.code ?? "UNKNOWN"}): ${root}`);
    }
}
...
async function readDirectorySorted(absoluteDir) {
    try {
 ...
 */
async function readRootDirectory(root, absoluteRoot) {
    try {
...
async function walk(absolutePath, absoluteRoot) {
    let stat;
    try {
...
export async function scanRepository(root) {
    ...
    if (rootStat.isSymbolicLink()) {
        throw new RepositoryScanError("INVALID_ROOT", root, `Repository path is a symbolic link; root symlinks are not followed: ${root}`);
    }
    if (!rootStat.isDirectory()) {
        throw new RepositoryScanError("NOT_A_DIRECTORY", root, `Repository path is not a directory: ${root}`);
    }
    const rootChildren = [];
